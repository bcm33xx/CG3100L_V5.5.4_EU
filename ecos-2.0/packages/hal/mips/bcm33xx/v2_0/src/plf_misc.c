...
static void   (*subIsr[32])( void *data )               = { NULL };
static void    *subIsrData[32]                          = { NULL };
...
static void hal_dsr_periph( cyg_vector_t    vector,
                            ...
                            cyg_addrword_t  data )
{
    cyg_uint32  activeInterrupts;
    cyg_uint32  currentInterrupt;
    int         i;

#if 0
    // When we initially brought up this HAL, we were getting spurious interrupts
    // for ISR 0 with no active peripheral interrupts.  This is a test to see
    // if that's still happening.
    static int  spuriousInterruptCount = 0;

    if (*INTC_IrqStatus & *INTC_IrqEnableMask == 0)
    {
        spuriousInterruptCount += 1;
        if (spuriousInterruptCount > 10)
        {
            spuriousInterruptCount = 0;
            diag_write_char('@');
        }
    }
#endif
    ...
    while (( activeInterrupts = *INTC_IrqStatus & *INTC_IrqEnableMask ) != 0 )
    {
#if 1
        // We start looking at the leftmost interrupt bit - bit 31.  This is
        // optimized by using assembler to skip leading zeroes:
        int  skipCount;
        asm( "clz %0, %1" : "=r" (skipCount) : "r" (activeInterrupts) );
        currentInterrupt = 1 << (31 - skipCount);

        for ( i = (31 - skipCount); i >= 0; i-- )
#else
...
#endif
        {
            ...
            {
                // Disable the interrupt.  The handler should re-enable it.
                *INTC_IrqEnableMask &= ~currentInterrupt;
                if (subIsr[i] != NULL)
                {
                    subIsr[i]( subIsrData[i] );
                }
            }
            currentInterrupt >>= 1;
        }
    ...
    // The manual says this has no effect.  It was added to try to eliminate the
    // spurious interrupts we're seeing.
