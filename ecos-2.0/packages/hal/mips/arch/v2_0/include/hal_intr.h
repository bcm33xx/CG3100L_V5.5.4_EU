...
#if 0 // This is the original version, using a fixed reset value.
...
// The interrupt service latency makes the clock run about 0.07% slow.
// Fix this by subtracting the latency from the clock reset value in the
// count register.  This reduces the latency to a few instructions.
// NOTE: This code assumes interlocked mfc0 and mtc0 instructions.
//   Read current count into $3
//   Read current compare into $2
//   Subtract to get latency
//   Subtract the latency from the desired reset value
//   If the result <= 0, use the desired reset value
//   Clear the count register in case count = new compare
//   Set the compare register
//   Clear the count register in case new compare < incremented count
#define HAL_CLOCK_RESET( _vector_, _period_ )   \
CYG_MACRO_START                                 \
    asm volatile (                              \
        "mfc0 $3, $9\n"                         \
