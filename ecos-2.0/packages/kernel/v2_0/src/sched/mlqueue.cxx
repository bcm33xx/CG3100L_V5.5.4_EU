...
Cyg_Scheduler_Implementation::Cyg_Scheduler_Implementation()
{
    ...
    for (HAL_SMP_CPU_TYPE j = 0; j < CYG_KERNEL_CPU_COUNT(); j++)
    {
        pending_map[j] = 0;
...
Cyg_Scheduler_Implementation::schedule(void)
{
    ...
    if( current->get_state() == Cyg_Thread::RUNNING )
    {
        ...
        for (HAL_SMP_CPU_TYPE i = 0; i < CYG_KERNEL_CPU_COUNT(); i++)
        {
            if ((current->cpu_affinity & (1 << i)) != 0)
            {
                pending[i][current->priority]++;
                pending_map[i] |= (1<<current->priority);
            }
        }
    }
    else
    ...
    HAL_LSBIT_INDEX(index, pending_map[cpu]);
    ...
    cyg_ucount32 affinity = 1 << cpu;
    while(( thread->cpu != CYG_KERNEL_CPU_NONE ) || ((thread->cpu_affinity & affinity) == 0))
    ...
    for (HAL_SMP_CPU_TYPE i = 0; i < CYG_KERNEL_CPU_COUNT(); i++)
    {
        if ((thread->cpu_affinity & (1 << i)) != 0)
        {
            if( --pending[i][index] == 0 )
                pending_map[i] &= ~(1<<index);
        }
    }
...
void Cyg_Scheduler_Implementation::set_need_reschedule(Cyg_Thread *thread)
{
    ...
    for ( unsigned i = 0; i < cpu_count; i++ )
    {
        ...
        // Add to this that the thread must have affinity to the CPU.

        Cyg_Thread *cur = current_thread[cpu];

        if( (!need_reschedule[cpu]) &&
            (cur->priority > thread->priority) &&
            ((thread->cpu_affinity & (1 << cpu)) != 0)
          )
        {
...
void Cyg_Scheduler_Implementation::set_idle_thread( Cyg_Thread *thread, HAL_SMP_CPU_TYPE cpu )
{
    ...
    // Give the thread affinity to its cpu.
    thread->set_affinity( 1<<cpu );
...
#ifdef CYGPKG_KERNEL_SMP_SUPPORT
// This should be defined in plf_smp.c or overridded in the application.
externC cyg_uint32 cyg_hal_smp_default_affinity;
#endif
...
)
{
    ...
    cpu_affinity = Cyg_Thread_fix_affinity( cyg_hal_smp_default_affinity );
#endif
