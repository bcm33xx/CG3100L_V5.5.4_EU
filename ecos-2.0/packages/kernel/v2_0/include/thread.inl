...
externC cyg_uint32 cyg_hal_smp_master_cpu;

// Swap affinity bits if cpu 0 isn't the master.
inline cyg_uint32 Cyg_Thread_fix_affinity( cyg_uint32 affinity_mask )
{
    if ( cyg_hal_smp_master_cpu != 0 )
    {
        // The default affinity assumes cpu 0 is the master.  If the master
        // cpu isn't cpu 0, we need to swap the affinity bits.  This is only
        // run once during thread creation, so there's no performance penalty.

        // Get the master and slave affinity bits.
        cyg_uint32 master_affinity = affinity_mask & (1<<0);
        cyg_uint32 slave_affinity  = affinity_mask & (1<<cyg_hal_smp_master_cpu);

        // Clear the master and slave affinity bits.
        affinity_mask = affinity_mask ^ (master_affinity | slave_affinity);

        // Shift the affinity bits to where they belong.
        affinity_mask |= (master_affinity << cyg_hal_smp_master_cpu) | (slave_affinity >> cyg_hal_smp_master_cpu);
    }
    return affinity_mask;
}

inline cyg_bool Cyg_Thread::set_affinity( cyg_uint32 affinity_mask )
{
    // If the affinity mask won't match any CPU, then the affinity value is invalid.
    if ( ( affinity_mask & ((1<<HAL_SMP_CPU_COUNT()) - 1) ) == 0 )
    {
        return false;
    }
    affinity_mask = Cyg_Thread_fix_affinity( affinity_mask );
    this->Cyg_SchedThread::cpu_affinity = affinity_mask;
    return true;
}

inline cyg_uint32 Cyg_Thread::get_affinity()
{
    cyg_uint32 affinity_mask = this->Cyg_SchedThread::cpu_affinity;
    affinity_mask = Cyg_Thread_fix_affinity( affinity_mask );
    return affinity_mask;
}
