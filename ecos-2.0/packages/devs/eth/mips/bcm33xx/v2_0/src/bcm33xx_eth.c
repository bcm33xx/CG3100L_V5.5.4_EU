...
//      bcm33xx_eth.c
//
//      Ethernet device driver for BCM33xx internal I/F
//
//==========================================================================
...
// Date:         2003-08-20
// Purpose:      
// Description:  hardware driver for BCM33xx ethernet
//              
//
//####DESCRIPTIONEND####
//
...
#include <pkgconf/devs_eth_mips_bcm33xx.h>
...
// BCM33xx LAN definitions
#include "bcm33xx_eth.h"
...
} bcm33xx_eth_priv_data;

ETH_DRV_SC(bcm33xx_eth_sc,
           &bcm33xx_eth_priv_data, // Driver specific data
           "eth0",             // Name for this interface
           bcm33xx_eth_start,
           bcm33xx_eth_stop,
           bcm33xx_eth_control,
           bcm33xx_eth_can_send,
           bcm33xx_eth_send,
           bcm33xx_eth_recv,
           bcm33xx_eth_deliver,     // "pseudoDSR" called from fast net thread
           bcm33xx_eth_poll,
           bcm33xx_eth_int_vector);

NETDEVTAB_ENTRY(bcm33xx_eth_netdev, 
                "bcm33xx_eth", 
                bcm33xx_eth_init, 
                &bcm33xx_eth_sc);

#ifdef CYGSEM_MIPS_BCM33XX_ETH_SET_ESA
    static unsigned char enaddr[] = CYGDAT_MIPS_BCM33XX_ETH_ESA;
#else
    static unsigned char enaddr[ETHER_ADDR_LEN];
#endif

static void bcm33xx_eth_poll(struct eth_drv_sc *sc);

// This ISR is called when the ethernet interrupt occurs
static int
bcm33xx_eth_isr(cyg_vector_t   vector, 
                cyg_addrword_t data)
{
    struct bcm33xx_eth_priv_data *spd   = (struct bcm33xx_eth_priv_data *)data;
    unsigned long                __base = spd->base;    

#ifdef DEBUG
    db_printf("bcm33xx_eth_isr\n");
#endif
    return (CYG_ISR_HANDLED|CYG_ISR_CALL_DSR);  // Run the DSR
}

static 
void bcm33xx_eth_dsr(cyg_vector_t   vector, 
                     cyg_ucount32   count, 
                     cyg_addrword_t data)
{
    struct bcm33xx_eth_priv_data *spd = (struct bcm33xx_eth_priv_data *)data;
...
static int
bcm33xx_eth_int_vector(struct eth_drv_sc *sc)
{
    struct bcm33xx_eth_priv_data *spd = (struct bcm33xx_eth_priv_data *)sc->driver_private;

    return spd->vector;
}

static void
__init_buffers(struct bcm33xx_eth_priv_data *spd)
{
    extern char  cyg_io_bcm33xx_eth_2kbuffers[];
    cyg_uint32  *bufp = (cyg_uint32 *)CYGARC_UNCACHED_ADDRESS((unsigned)cyg_io_bcm33xx_eth_2kbuffers);
    int          i;
    int          j;
...
static void
__select_buffer(struct bcm33xx_eth_priv_data *spd, 
                int                           buf_nr)
{
    unsigned long   __base = spd->base;
    cyg_uint32     *p;
...
static void
__init_cam(struct bcm33xx_eth_priv_data *spd)
{
    unsigned long   __base = spd->base;
...
static void
__stop_dma(struct bcm33xx_eth_priv_data *spd)
{
    unsigned long  __base = spd->base;
...
static void
__init_dma(struct bcm33xx_eth_priv_data *spd)
{
    unsigned long  __base = spd->base;
...
static void
__check_mii(struct bcm33xx_eth_priv_data *spd)
{
    unsigned long  __base   = spd->base;
    cyg_uint32     opmode;
...
static void
bcm33xx_eth_reset(struct bcm33xx_eth_priv_data *spd)
{
    unsigned long  __base = spd->base;
...
static bool 
bcm33xx_eth_init(struct cyg_netdevtab_entry *tab)
{
...
static void
bcm33xx_eth_stop(struct eth_drv_sc *sc)
{
    struct bcm33xx_eth_priv_data  *spd      = (struct bcm33xx_eth_priv_data *)sc->driver_private;
    unsigned long                  __base   = spd->base;
...
static void
__do_start(struct bcm33xx_eth_priv_data *spd)
{
    unsigned long  __base = spd->base;
    int            i;
...
//
static void
bcm33xx_eth_start(struct eth_drv_sc  *sc, 
...
//
static int
bcm33xx_eth_control(struct eth_drv_sc  *sc, 
                    unsigned long       key,
                    void               *data, 
                    int                 data_length)
{
...
//
static int
bcm33xx_eth_can_send(struct eth_drv_sc *sc)
{
    struct bcm33xx_eth_priv_data *spd    = (struct bcm33xx_eth_priv_data *)sc->driver_private;
    unsigned long                 __base = spd->base;
...
static void 
bcm33xx_eth_send(struct eth_drv_sc *sc, 
...
#ifdef DEBUG
    db_printf("bcm33xx_eth_send: %d sg's, %d bytes, KEY %x\n",
...
static void
__check_rxstate(struct bcm33xx_eth_priv_data *spd)
{
    unsigned long   __base = spd->base;
...
static void
bcm33xx_eth_recv(struct eth_drv_sc *sc, 
                 struct eth_drv_sg *sg_list, 
                 int                sg_len)
{
    struct bcm33xx_eth_priv_data *spd = (struct bcm33xx_eth_priv_data *)sc->driver_private;
...
static void
bcm33xx_eth_poll(struct eth_drv_sc *sc)
{
    struct bcm33xx_eth_priv_data *spd = (struct bcm33xx_eth_priv_data *)sc->driver_private;

    cyg_drv_interrupt_mask(spd->vector);

    (void)bcm33xx_eth_isr(spd->vector, (cyg_addrword_t)spd);
...
static void
bcm33xx_eth_deliver(struct eth_drv_sc *sc)
{
    struct bcm33xx_eth_priv_data *spd    = (struct bcm33xx_eth_priv_data *)sc->driver_private;
    unsigned long                 __base = spd->base;    
