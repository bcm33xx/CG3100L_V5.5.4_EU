...
eth_drv_recv(struct eth_drv_sc *sc, int total_len)
{
    ...
    if (m == 0) {
        ...
        // BRCM ADD - RTE - keep track of our socket state
        //diag_printf("Storing off stocket info..\n");
        //_store_socket_history();

        // BRCM ADD - RTE - run our recovery mechanism
        //diag_printf("Executing recovery procedures...\n");
        //_flush_sockets();
...
#ifdef CYGPKG_IO_ETH_DRIVERS_WARN_NO_MBUFS
                START_CONSOLE();
                diag_printf("warning: eth_recv out of Clusters\n");
