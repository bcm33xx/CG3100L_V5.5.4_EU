...
#endif        
        // initalize our total block counter
        chan->out_cbuf.blocking_counter = 0;
    }
...
// Throw away any pending output
    ...
    case CYG_IO_GET_CONFIG_SERIAL_BLOCKING_COUNT:
    {
        // sanity check and extraction of blocking counter
        if (xbuf != NULL)
            *(cyg_uint32 *)xbuf = out_cbuf->blocking_counter;
        else 
            return -EINVAL;
    }
