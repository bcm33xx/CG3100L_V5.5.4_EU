...
// begin cliffmod
/* Allocate hent structure with room for num_list_elements in_addrs. 
   Returns NULL on failure. */
static struct hostent*
alloc_hent_n( int num_list_elements )
{
    struct hostent *hent;
    int idx;
    ...
    if (hent) 
    {
        ...
        hent->h_addr_list = malloc( sizeof(char*)*num_list_elements );
        if( !hent->h_addr_list ) 
        {
            #if DMP_STACK_DEBUG
            diag_printf( "alloc_hent() --> malloc of hent->h_addr_list failed.\n");
            #endif
        ...
        for( idx = 0; idx < num_list_elements; idx++ )
        {
            hent->h_addr_list[ idx ] = malloc( sizeof(struct in_addr) );
            if( !hent->h_addr_list[ idx ] ) 
            {
                #if DMP_STACK_DEBUG
                diag_printf( "alloc_hent() --> malloc of hent->h_addr_list[ %x ] failed.\n", idx );
                #endif
            ...
            {
                #if DMP_STACK_DEBUG
                diag_printf( "alloc_hent() --> hent->h_addr_list[ %x ]=0x%p\n", 
                    idx, hent->h_addr_list[ idx ] );
                #endif
            }
        }

        hent->h_addr_list[ num_list_elements - 1 ] = NULL;
    }
 ...
 */
int  
cyg_dns_res_init(struct in_addr *dns_server) __attribute__ ((weak));
int  
