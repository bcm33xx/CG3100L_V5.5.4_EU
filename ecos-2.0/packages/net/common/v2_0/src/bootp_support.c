...
init_net(const char *intf, struct bootp *bp)
{
    ...
    unsigned int length;
    unsigned int bcm_net_if_num = 0;    // cliffmod

    // begin cliffmod
    diag_printf( "inet_net() - entering..." );
    if( intf )
    {
        diag_printf( "for network interface %s", intf );
    }
    diag_printf( " \n" );
    // end cliffmod
    ...
    if (get_bootp_option(bp, TAG_GATEWAY, &gateway,&length)) {
        ...
        if ( 0 != gateway.s_addr ) {
            // begin cliffmod
#ifdef MULTI_INF_SAME_SUBNET
            if( intf )
            {
                // is net interface name associated with a broadcom 
                // network interface?
                bcm_net_if_num = BcmNetIfNameToNetIfNum( intf );
                diag_printf( "inet_net() - net interface name: %s  bcm_net_if_num: %d\n",
                    intf, bcm_net_if_num );

                if( BcmNetIfNumIsValid( bcm_net_if_num ) )
                {
                    // valid broadcom net interface num specified. --> use custom routing...
                    BcmSetDefaultIpGateway( bcm_net_if_num, gateway.s_addr );
                    BcmShowDefaultIpGateway();
                }
            } // end if( intf )
#endif
            // end cliffmod
